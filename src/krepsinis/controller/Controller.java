package krepsinis.controller;

import krepsinis.model.Krepsinis;
import krepsinis.model.KrepsinisDAO;
import krepsinis.model.User;
import krepsinis.model.UserDAO;
import krepsinis.utils.Validation;
...
public class Controller {
    ...
    @FXML
    private CheckBox cbVarle;
    @FXML
    private CheckBox cbVaignora;
    @FXML
    private RadioButton rbAlyga;
    @FXML
    private RadioButton rbBlyga;
    @FXML
    private RadioButton rbClyga;
    @FXML
    private RadioButton rbIol;
    @FXML
    private RadioButton rbtrlyga;
    @FXML
    private ComboBox teamsNum;
    @FXML
    ...
    public void registerLogin(ActionEvent event) {
        ...
        if (!Validation.isValidUsername(regUser.getText())) {
            ...
            regError.setText("Email is not correct, pattern- krepsinis@one.lt");
            isRegistered = false;
        }
    ...
    public void create() {
        ...
        String sponsors = "";
        if (cbTOP.isSelected()) {
            sponsors += cbTOP.getText() + ",";
        }
        if (cbVarle.isSelected()) {
            sponsors += cbVarle.getText() + ",";
        }
        if (cbVaignora.isSelected()) {
            sponsors += cbVaignora.getText() + ",";
        }

        String league = "";
        if (rbAlyga.isSelected()) {
            league += rbAlyga.getText();
        } else if (rbBlyga.isSelected()) {
            league += rbBlyga.getText();
        } else if (rbClyga.isSelected()) {
            league += rbClyga.getText();
        }else if(rbIol.isSelected()){
            league+=rbIol.getText();
        }else if(rbtrlyga.isSelected()){
            league+=rbtrlyga.getText();
        }
        String teams = "";
        if (!teamsNum.getSelectionModel().isEmpty()) {
            teams = teamsNum.getSelectionModel().getSelectedItem().toString();
        } else {
            warning.setText("Please check team numbers");
        }
        ...
        if (!Validation.isValidTeamName(teamName1)) {
            ...
            Krepsinis krepsinis = new Krepsinis(teamName1, nameSurname1, league, sponsors, Integer.parseInt(teams), user.getId());
            KrepsinisDAO dakarDAO = new KrepsinisDAO();
            String msg = dakarDAO.add(krepsinis);
            warning.setText(msg);
    ...
    public void updateTableFromDB(String teamName) {
        KrepsinisDAO krepsinisDAO = new KrepsinisDAO();
        UserDAO userDAO = new UserDAO();
        User user = userDAO.getUser(logname.getText());
        try {
            rsAllEntries = krepsinisDAO.searchByTeamName(teamName, user);
    ...
    public void update() {
        if(role.getText().equals("Admin")) {
            ...
            if (!Validation.isValidTeamName(teamName1)) {
                ...
                Krepsinis krepsinis = new Krepsinis(teamid, teamName1, nameSurname1, league, sponsors, Integer.parseInt(teams), Integer.parseInt(userid));
                KrepsinisDAO krepsinisDAO = new KrepsinisDAO();
                krepsinisDAO.editById(krepsinis);
    ...
    public void delete() {
        if (role.getText().equals("Admin")) {
            KrepsinisDAO krepsinisDAO = new KrepsinisDAO();
            krepsinisDAO.deleteById(Integer.parseInt((String) (id.getText())));
